...
setValorST(char *nomeVar, CelObjeto *pValor)
{
    ...
    if(found == NULL){
        aux = ini;
        found = mallocSafe(sizeof(*found));
        while(aux->proxVar != NULL) aux = aux->proxVar;
        found->valorVar.vFloat = pValor->valor.vFloat;
        found->tipoVar = pValor->categoria;
        found->nomeVar = mallocSafe((strlen(nomeVar) + 1) * sizeof(char));
...
endVarST(char *nomeVar)
{
    ...
    */
    ...
    while(celVar != NULL &&  found == 0){
        if(strcmp(nomeVar, celVar->nomeVar) == 0) found = 1;
        if(found == 0) celVar = celVar->proxVar;
    }
    return (celVar);
}
